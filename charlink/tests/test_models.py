...
class TestAppSettings(TestCase):
    def test_str_method(self):
        ...
        app_setting = AppSettings.objects.get(
    ...
    def test_str_method_non_existent_app(self):
        AppSettings.objects.create(
            app_name="memberaudit_default", default_selection=True
        )
...
class TestBaseFilter(TestCase):
    ...
    def setUpTestData(cls):
        cls.base_filter = BaseFilterTestImpl(
...
class TestComplianceFilter(TestCase):
    ...
    def test_process_filter(self, mock_audit_filter):
        mock_audit_filter.return_value = {
            self.user.pk: {"check": True, "message": "All good"}
        }
    ...
    def test_audit_filter_simple(self):
        import_apps()
        self.compliance_filter.selected_apps.add(
            AppSettings.objects.get(app_name="allianceauth.authentication_default")
        )
        res = self.compliance_filter.audit_filter(User.objects.filter(pk=self.user.pk))
        self.assertDictEqual(
            res, {self.user.pk: {"check": True, "message": "Meets requirements"}}
        )
    ...
    def test_audit_filter_with_2_apps(self, mock_update_character):
        ...
        self.compliance_filter.selected_apps.add(
            AppSettings.objects.get(app_name="corptools_default")
        )

        res = self.compliance_filter.audit_filter(User.objects.filter(pk=self.user.pk))
        self.assertDictEqual(
            res,
            {self.user.pk: {"check": False, "message": "Does not meet requirements"}},
        )

        token2 = add_new_token(
            self.user, character, scopes=corptools_import.get("default").scopes
        )
        ...
        main_token = Token.objects.get(
            user=self.user, character_id=self.user.profile.main_character.character_id
        )
    ...
    def test_audit_filter_removed_app(self):
        new_as = AppSettings.objects.create(
            app_name="non_existent_app", default_selection=True
        )
