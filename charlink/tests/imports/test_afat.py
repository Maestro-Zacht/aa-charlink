...
class TestIsCharacterAdded(TestCase):
    ...
    def test_ok(self):
        ...
        self.assertFalse(
            app_import.get("readfleet").is_character_added(self.main_character)
        )
        self.assertTrue(
            app_import.get("readfleet").is_character_added(self.char_readfleet)
        )
        self.assertFalse(
            app_import.get("readfleet").is_character_added(self.char_clickfat)
        )

        self.assertFalse(
            app_import.get("clickfat").is_character_added(self.main_character)
        )
        self.assertFalse(
            app_import.get("clickfat").is_character_added(self.char_readfleet)
        )
        self.assertTrue(
            app_import.get("clickfat").is_character_added(self.char_clickfat)
        )
...
class TestCheckPermissions(TestCase):
    ...
    def test_ok(self):
        ...
        self.assertTrue(
            app_import.get("readfleet").check_permissions(self.add_fatlink_user)
        )
        self.assertTrue(app_import.get("readfleet").check_permissions(self.both_user))
        self.assertFalse(
            app_import.get("readfleet").check_permissions(self.no_perm_user)
        )

        self.assertFalse(
            app_import.get("clickfat").check_permissions(self.no_perm_user)
        )
        self.assertTrue(
            app_import.get("clickfat").check_permissions(self.basic_access_user)
        )
...
class TestGetUsersWithPerms(TestCase):
    ...
    def test_ok(self):
        ...
        self.assertQuerySetEqual(
            users_readfleet,
            [self.manage_user.pk, self.add_fatlink_user.pk, self.both_user.pk],
            ordered=False,
        ...
        self.assertQuerySetEqual(
            users_clickfat, [self.basic_access_user.pk], transform=lambda x: x.pk
        )
