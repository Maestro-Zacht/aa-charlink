...
from charlink.app_imports import (
    get_duplicated_apps,
...
class TestLoginImport(TestCase):
    def test_get_query_id(self):
        ...
        self.assertEqual(
            login_import.get_query_id(), "allianceauth.authentication_default"
        )

    def test_hash(self):
        login_import = import_apps()["allianceauth.authentication"].get("default")
        self.assertEqual(
            hash(login_import), hash("allianceauth.authentication_default")
        )

    def test_is_ignored(self):
        marketmanager = import_apps()["marketmanager"]

        AppSettings.objects.filter(app_name="marketmanager_corporation").update(
            ignored=True
        )
...
class TestAppImport(TestCase):
    ...
    def test_get_form_fields_ignore(self):
        ...
        AppSettings.objects.filter(
            Q(app_name="marketmanager_corporation")
            | Q(app_name__startswith="corptools")
        ).update(ignored=True)
    ...
    def test_get_imports_with_perms_ignore(self):
        ...
        imports_marketmanager = marketmanager_import.get_imports_with_perms(
            user_marketmanager
        )
    ...
    def test_validate_import(self):  # noqa: PLR0915
        app_import = import_apps()["allianceauth.authentication"]
