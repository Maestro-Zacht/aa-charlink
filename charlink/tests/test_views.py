...
class TestDashboardPost(TestCase):
    ...
    def test_post_ok(self):
        ...
        self.assertRedirects(
            res, reverse("charlink:login"), fetch_redirect_response=False
        )
...
class TestLoginView(TestCase):
    ...
    def setUpTestData(cls):
        cls.scopes = list(
            set(
                corptools_import.imports[0].scopes
    ...
    def test_ok(
        ...
        mock_token_required,
        mock_import_apps,
        mock_corptools_add_character,
        mock_miningtaxes_add_character,
    ):
        import_apps()
        session = self.client.session
        session["charlink"] = {
            ...
            "corptools": AppImport(
                ...
                [
                    LoginImport(
                        ...
                        is_character_added=corptools_import.imports[
                            0
                        ].is_character_added,
                        is_character_added_annotation=corptools_import.imports[
                            0
                        ].is_character_added_annotation,
                        get_users_with_perms=corptools_import.imports[
                            0
                        ].get_users_with_perms,
                    )
                ],
            ),
            "miningtaxes": AppImport(
                "miningtaxes",
                [
                    LoginImport(
                        ...
                        check_permissions=miningtaxes_import.imports[
                            0
                        ].check_permissions,
                        is_character_added=miningtaxes_import.imports[
                            0
                        ].is_character_added,
                        is_character_added_annotation=miningtaxes_import.imports[
                            0
                        ].is_character_added_annotation,
                        get_users_with_perms=miningtaxes_import.imports[
                            0
                        ].get_users_with_perms,
                    )
                ],
            ),
            "allianceauth.authentication": AppImport(
                ...
                [
                    LoginImport(
                        ...
                        is_character_added=lambda character: (
    ...
    def test_ignore(
        ...
        import_apps()
        AppSettings.objects.filter(app_name__startswith="miningtaxes").update(
            ignored=True
        )
...
class TestAudit(TestCase):
    ...
    def test_ok(self):
        ...
        res = self.client.get(
            reverse("charlink:audit_corp", args=[self.corp.corporation_id])
        )
    ...
    def test_no_perm(self):
        ...
        res = self.client.get(
            reverse("charlink:audit_corp", args=[self.corp2.corporation_id])
        )
...
class TestSearch(TestCase):
    ...
    def test_ok(self):
        ...
        res = self.client.get(
            reverse("charlink:search"), {"search_string": self.main_char.character_name}
        )
    ...
    def test_not_found(self):
        ...
        res = self.client.get(
            reverse("charlink:search"),
            {"search_string": self.main_char2.character_name},
        )
...
class TestAuditApp(TestCase):
    ...
    def test_ok(self):
        ...
        self.assertEqual(len(next(iter(res.context["logins"].values()))), 2)

    def test_app_empty_perms(self):
        self.client.force_login(self.user)

        res = self.client.get(
            reverse("charlink:audit_app", args=["allianceauth.authentication"])
        )

        self.assertEqual(res.status_code, 200)
        self.assertIn("logins", res.context)
        self.assertEqual(len(res.context["logins"]), 1)
        self.assertEqual(len(next(iter(res.context["logins"].values()))), 3)
        self.assertIn("app", res.context)
...
class TestAdminImportedApps(TestCase):
    ...
    def test_admin(
        self,
        mock_no_import,
        mock_failed_to_import,
        mock_duplicated_apps,
        mock_import_apps,
    ):
...
class TestToggleAppVisible(TestCase):
    ...
    def test_toggle_authentication_login(self):
        ...
        app_settings = AppSettings.objects.create(
            app_name=app_name, default_selection=True
        )
...
class TestToggleAppDefaultSelection(TestCase):
    ...
    def test_toggle_authentication_login(self):
        ...
        res = self.client.get(
            reverse("charlink:toggle_app_default_selection", args=[app_name])
        )
    ...
    @patch("charlink.views.import_apps")
    @patch("charlink.app_imports.import_apps")
    def test_toggle_other_login_default_true(
        self, mock_views_import_apps, mock_app_imports_import_apps
    ):
        mock_import_apps = {
            "testapp": AppImport(
                "testapp",
    ...
    @patch("charlink.views.import_apps")
    @patch("charlink.app_imports.import_apps")
    def test_toggle_other_login_default_false(
        self, mock_views_import_apps, mock_app_imports_import_apps
    ):
        mock_import_apps = {
            "testapp": AppImport(
        ...
        app_settings = AppSettings.objects.create(
            app_name=app_name, default_selection=False
        )
