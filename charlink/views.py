...
def get_navbar_elements(user: User):
    is_auditor = (
        user.has_perm("charlink.view_state")
        or user.has_perm("charlink.view_corp")
        or user.has_perm("charlink.view_alliance")
    )
...
def dashboard_login(request):
    ...
    return render_to_string(
        "charlink/dashboard_login.html", context=context, request=request
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form_field_pattern = re.compile(
                r"^(?P<app>[\w\d\.]+)_(?P<unique_id>[a-zA-Z0-9]+)$"
            )
...
def login_view(request, token):
    ...
    for app, unique_id in charlink_data["imports"]:
        ...
        if (
            app != "allianceauth.authentication"
            and not import_.is_ignored
            and import_.check_permissions(request.user)
        ):
            try:
                import_.add_character(request, token)
            except Exception:
                logger.exception("Failed to add character to %s", import_.field_label)
                messages.error(
                    request,
                    _("Failed to add character to %(field_label)s")
                    % {"field_label": import_.field_label},
                )
            else:
                messages.success(
                    request,
                    _("Character successfully added to %(field_label)s")
                    % {"field_label": import_.field_label},
                )
...
def audit(request, corp_id: int):
    ...
    if not corps.filter(corporation_id=corp_id).exists():
        raise PermissionDenied(
            _("You do not have permission to view the selected corporation statistics.")
        )
...
def audit_user(request, user_id):
    ...
    if (
        ...
        and not corps.filter(
            corporation_id=user.profile.main_character.corporation_id
        ).exists()
    ):
        raise PermissionDenied(
            _("You do not have permission to view the selected user statistics.")
        )
...
def audit_app(request, app):
    ...
    if not app_imports.has_any_perms(request.user):
        raise PermissionDenied(
            _("You do not have permission to view the selected application statistics.")
        )
    ...
    for import_ in app_imports.imports:
        visible_characters = EveCharacter.objects.filter(
            (
                ...
                | Q(
                    character_ownership__user__profile__main_character__corporation_id__in=corp_ids
                )
            )
...
def toggle_app_default_selection(request, app_name):
    if app_name == "allianceauth.authentication_default":
        messages.error(
            request,
            _(
                "The default selection for the built-in AllianceAuth login cannot be changed."
            ),
        )
    else:
        app_settings = get_object_or_404(AppSettings, app_name=app_name)
        ...
        messages.success(
            ...
            % {
                "app_name": app_settings,
                "selected": _("selected")
                if app_settings.default_selection
                else _("not selected"),
            },
        )

        if (
            not login_import.default_initial_selection
            and app_settings.default_selection
        ):
            messages.warning(
                request,
                format_html(
                    ...
                    + "</h1><br>"
                    + _(
                        "The developer set this to not selected in order to avoid ESI bans. Ensure you know what you are doing."
                    )
                ),
            )
